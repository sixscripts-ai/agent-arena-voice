    ...
    <main className="min-h-screen bg-[#11110f] font-sans text-[#e8e6de] selection:bg-[#2a5cff] selection:text-white">
      ...
      <section className="grid min-h-[calc(100vh-3.5rem)] grid-cols-1 xl:grid-cols-[280px_minmax(0,1fr)_320px]">
        ...
        <section className="min-w-0 border-b border-[#3a3932] bg-[#11110f] xl:border-b-0 xl:border-r">
          <div className="flex h-11 items-center justify-between border-b border-[#3a3932] px-4"><div className="flex h-full items-end gap-4"><span className="pb-3 font-mono text-[10px] uppercase tracking-[0.16em] text-[#9b9a91]">Artifact workspace</span>{(["source", "tests", "diff"] as ArtifactTab[]).map(item => <button key={item} onClick={() => setTab(item)} className={`h-full border-b-2 px-1 pt-1 font-mono text-[10px] uppercase tracking-[0.14em] ${tab === item ? "border-[#d1a251] text-[#f1d08d]" : "border-transparent text-[#89877f] hover:text-[#e8e6de]"}`}>{item}</button>)}</div><span className="font-mono text-[9px] text-[#74736c]">fixture / policy-defense</span></div>
          <div className="grid min-h-[calc(100%-2.75rem)] grid-cols-[150px_minmax(0,1fr)]">
            ...
            <div className="min-w-0 overflow-auto p-5"><div className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2 text-xs text-[#bcbab0]"><Code2 className="h-4 w-4 text-[#d1a251]" />{tab === "source" ? "solution.ts" : tab === "tests" ? "policy.test" : "change.diff"}</div><span className="font-mono text-[9px] text-[#74736c]">read-only fixture</span></div><pre className={`overflow-x-auto whitespace-pre rounded-none border border-[#36352f] bg-[#0c0c0b] p-5 font-mono text-[12px] leading-6 ${tab === "diff" ? "text-[#e1d4bd]" : "text-[#d6d5ce]"}`}>{content}</pre></div>
          </div>
        </section>
